...
def main():
    ...
    a,b,c = st.columns(3)
    with a:
        timeframe = st.radio("Select timeframe (minutes)", ['1m', '3m', '5m', '15m', '30m', '45m', '1h'])    
    with b:
        lookback_period = st.radio("Select lookback period (minutes)", [30, 60, 120, 180, 240, 360, 1440])        
    with c:
        nlargest = st.radio("Select number of pairs to display", [5, 10, 15, 20, 25, 50, 100])  
